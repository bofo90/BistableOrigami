...
totalnumberHinges = 54
totalnumberEdges = 216
...
#    ax6.legend(loc = 2, fontsize =15, framealpha = 0.5, edgecolor = 'inherit', fancybox = False
    ...
    divisions = 10
    colors = cm.rainbow(np.linspace(0, 1, divisions))
...
#        findit = np.where(differentEnergies[:,0] == hinge)[0]
        if len(np.where(differentEnergies[:,0] == hinge)[0]) != 0:
            col = '#FE9128'
#            c = colors[int(differentEnergies[findit[0],1]%divisions)]
            ax1.scatter(eEdgeRel[stepsHinge*hinge+stepsHinge-1], eHingeRel[stepsHinge*hinge+stepsHinge-1], c = col, label = hingeName[hinge])
