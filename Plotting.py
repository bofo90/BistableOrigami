...
def StableStatesCounturPlotPaper2(allDesigns,x, xname, y, yname, z, color, colorName, colormap, minmax, save = False, Folder_name = '', NameFig = ''):
    ...
    minCol = 0#minmax[0]#np.min(allDesigns.iloc[:,color])
    maxCol = 1#minmax[1]#np.max(allDesigns.iloc[:,color])
...
right=0.945)
    ...
    cmaptemp = matl.cm.get_cmap(colormap)
    cmapfig, normfig = from_levels_and_colors(np.linspace(minCol, maxCol, 1000),cmaptemp(np.linspace(0, 1,999)))
    
    fig1 = plt.figure(figsize=(cm2inch(1.5), cm2inch(2.8)))
    fig1.subplots_adjust(top=0.954,
bottom=0.041,
left=0.0,
right=0.911)
    ax1 = plt.subplot(111)
    cbar = plt.colorbar(matl.cm.ScalarMappable(norm=normfig, cmap=cmapfig), ax = ax1, 
                        fraction=0.99, pad=0.01, orientation='vertical', aspect=15)
    cbar_ticks = np.linspace(minCol,maxCol,3)
    # cbar_ticks_names = [r'$10^{%d}$' %label for label in cbar_ticks]
    cbar.set_ticks(cbar_ticks)
    # cbar.ax.set_yticklabels(cbar_ticks_names)
    cbar.set_label(colorName, fontsize = 9, color = '0.2',labelpad = 0)
