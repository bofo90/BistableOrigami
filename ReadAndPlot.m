            ...
            for ct = 1:length(allFiles)
                ...
                if strcmp(opt.readHingeFile,'off')
                    if ~isequal(hingeSet, opt.angleConstrFinal(end).val(:,1))
                        continue;
                    elseif ~strcmp(resfilename(1:end-4), '[3 24]_Ang1_17_Angl2_7')
                        continue;
                    end
                end
...
headersEnergy = {'Hinge Number'; 'EdgeEnergyFol'; 'EdgeEnergyRel'; ...
    'DiagonalEnergyFol'; 'DiagonalEnergyRel'; 'FaceEnergyFol'; 'FaceEnergyRel'; ...
