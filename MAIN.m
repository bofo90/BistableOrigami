...
opt=initOpt('inputType','individual', 'plot','result',... 
            ...
            'Khinge',0.001*10^(ksim(1)/4),'Kedge',1*10^((ksim(3)-1)/2),'Kface',1,'KtargetAngle',10^((ksim(2)-4)/4),...
            'stepkHinge', 1, 'stepkTargetAngle', 3, 'stepkEdge', 1,...
            'stepMaxStrech', 1, 'maxStretch', nan);

opt.saveFile = strcat('/',date,'_parameterAnalysisless3');
%         opt.saveFile = '/15-Dec-2017_100AngleCnstr';

%-pi if its the extruded version, pi if its
%only the internal polyheron
hingeSet = [12 7 8 13 21 26 22 30];%[7 3 8 13 17 26 22 30];
