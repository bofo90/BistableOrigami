...
internalHinges = 9
flags = np.empty((9, 10, 4, 6)) #6 possible flags
flags[:] = np.NaN
ststs = np.empty((9, 10, 4))
ststs[:] = np.NaN
durations = np.empty((9, 10, 4))
durations[:] = np.NaN
folder_name = "Results/tetrahedron/sqp/energy/"
folder_name2 = "Results/tetrahedron/sqp/mat/"
...
for stephinge in np.arange(9):
    khinge = 0.001*10**(stephinge/4);
    for stepangle in np.arange(10):
        kangle = 0.01*10**(stepangle/4);
        for stepedge in np.arange(4):
...
x = 0.001*10**(np.arange(5)/2)
y = 0.01*10**(np.arange(10)/4)
edges = 0.1*10**(0.5+np.arange(4)/2)
x2 = 0.001*10**(np.arange(10)/4-0.125)
y2 =0.01*10**(np.arange(11)/4-0.125)

fig1, axes1 = plt.subplots(nrows=1, ncols=4, figsize=(pc2.cm2inch(48), pc2.cm2inch(15)))
fig1.subplots_adjust(wspace = 0, hspace = 0.15, left = 0.06, right = 0.98, top = 0.94, bottom = 0.12)
...
for edge, ax in zip(np.arange(4), axes1.flat):
...
fig2, axes2 = plt.subplots(nrows=1, ncols=4, figsize=(pc2.cm2inch(48), pc2.cm2inch(15)))
fig2.subplots_adjust(wspace = 0, hspace = 0.15, left = 0.06, right = 0.98, top = 0.94, bottom = 0.12)
...
cmap2, norm2 = from_levels_and_colors(np.linspace(0, 2, 6), cm.Blues_r(np.linspace(0, 1, 5)))
cmap2.set_over('0.2')

for edge, ax in zip(np.arange(4), axes2.flat):
    cs2 = ax.pcolormesh(x2, y2, durations[:,:,edge].T, cmap=cmap2, norm = norm2)

cbar2 = plt.colorbar(cs2, format="%.1f", ax =axes2.ravel().tolist(), fraction=0.05, pad=0.01)
...
goodstst = np.zeros((9,10,4))
goodststperst = np.empty((0,9,10,4), dtype = int)
...
fig4, axes4 = plt.subplots(nrows=1, ncols=4, figsize=(pc2.cm2inch(48), pc2.cm2inch(15)))
fig4.subplots_adjust(wspace = 0, hspace = 0.15, left = 0.06, right = 0.98, top = 0.94, bottom = 0.12)
...
for edge, ax in zip(np.arange(4), axes4.flat):
...
fig5, axes5 = plt.subplots(nrows=1, ncols=4, figsize=(pc2.cm2inch(48), pc2.cm2inch(15)))
fig5.subplots_adjust(wspace = 0, hspace = 0.15, left = 0.06, right = 0.98, top = 0.94, bottom = 0.12)
...
for state, c in zip(np.arange(goodststperst.shape[0]), colors):
    for edge, ax in zip(np.arange(4), axes5.flat):
        prov2 = ax.scatter(0.001*10**(np.nonzero(goodststperst[state,:,:,edge])[0]/4), 0.01*10**(np.nonzero(goodststperst[state,:,:,edge])[1]/4),
